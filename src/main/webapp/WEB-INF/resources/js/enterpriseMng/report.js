...
function putReportData(reportData, reportType){
    ...
    //初始化报表导航菜单-默认选中
    fnTabCar('#card_btn li','#reportVal .table_content_son');
}
/******************************** 很重要的map start *********************************/
...
function createReportModel(data, reportType){
    ...
    for (var i = 0;i < data.length;i++) {
        ...
        htmlContent += '<a href="javaScript:;" onclick="exportReportDataExcel('+sheetId+', '+(i+1)+');">导出数据</a>';
        htmlContent += '<a href="javaScript:;" onclick="importReportDataExcel('+sheetId+');">导入数据</a>';
        htmlContent += '<a href="javaScript:;" onclick="downLoadReportExcelModel('+sheetId+', '+(i+1)+');">导出模版</a>';
        htmlContent += '</div>';*/
...
function createReportSheetHtml(reportType, sheetId, sheetRowData, columnNoFlag) {
    ...
    if (reportFormulaMap[sheetId] != null) {
        if (reportFormulaMap[sheetId][sheetRowData.modleId]) {
            //带有公式字段
            htmlContent += '<td class="statement_center">';
            htmlContent += '<input type="text" id="'+rowId+'_begin" data-id="'+beginExcelName+'" style="text-align: center;" data-readonly="1" name="'+rowId+'" value="0" />';
            htmlContent += '</td>';
            htmlContent += '<td class="statement_center">';
            htmlContent += '<input type="text" id="'+rowId+'_end" data-id="'+endExcelName+'" style="text-align: center;" data-readonly="1" name="'+rowId+'" value="0" />';
            htmlContent += '</td>';
        } else {
            htmlContent += '<td class="statement_center">';
            htmlContent += '<input type="text" id="'+rowId+'_begin" data-id="'+beginExcelName+'" style="text-align: center;" onchange="value=validateNumber(this);" name="'+rowId+'" value="" />';
            htmlContent += '</td>';
            htmlContent += '<td class="statement_center">';
            htmlContent += '<input type="text" id="'+rowId+'_end" data-id="'+endExcelName+'" style="text-align: center;" onchange="value=validateNumber(this);" name="'+rowId+'" value="" />';
            htmlContent += '</td>';
        }
    } else {
        htmlContent += '<td class="statement_center">';
 ...
 * @param formId 表单id
 */
function formulaFun(sheetId, formId){
 ...
 */
function allReportVal(data, reportType){
    var reportData = data;
    //清空当前值
    removeEntReportVal();
    //报表概况表赋值
