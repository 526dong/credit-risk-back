...
function addTr(obj) {
    ...
    var mins = $("input[name='valueMins']");
    var maxs = $("input[name='valueMaxs']");
    if (mins.length==1){
        $(maxs[0]).attr("placeholder","不填写表示正无穷");
        $(mins[0]).attr("placeholder","不填写表示负无穷");
    }else if (mins.length>1){
        for(var i = 0; i<mins.length-1;i++){
            $(maxs[i]).attr("placeholder","");
            $(mins[i+1]).attr("placeholder","");
        }
    }
...
function delTr(obj, id) {
    ...
    if($("input[name='valueMins']").length == 1){
        $("input[name='valueMins']:first").attr("placeholder","不填写表示负无穷");
        $("input[name='valueMaxs']:last").attr("placeholder","不填写表示正无穷");
    }
}
//显示定性
function showNature(){
